...
- **Two-tier Layer 2 (Tier A structural + Tier B evidence-based, total cap 7)**: Splits Layer 2 into two tiers to address under-inference (1-3 items) for senior professionals. Tier A (≤3) covers role-standard capabilities deduced from seniority alone (Partner/Director/MD/VP/C-suite) — requires only a seniority signal (≥1 criterion), is limited to a well-established function-level capability table, and cites the title as trigger. Tier B (≤4) is the existing evidence-based path requiring ≥2 criteria (seniority, quantifiable result, role expectation, or specific engagement evidence). Total cap raised from 5 → 7. Root cause: the ≥2 confidence threshold filtered out structurally obvious inferences that any practitioner would expect — e.g. "Financial Planning & Analysis" for a CFO, "Deal Origination" for an M&A Partner — because those capabilities are so universal they rarely appear as explicit profile claims, leaving the inference gate underpopulated for experienced professionals.
